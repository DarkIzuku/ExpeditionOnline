...
auto parse_options(int argc, char **argv) -> Options {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--host")
      ...
      std::cout << "ExpeditionOnlineSelfTest [--host address] [--port number] "
...
auto player_id_of(const proto::Frame &frame) -> std::optional<std::uint64_t> {
  ...
  case proto::MessageType::welcome:
    return proto::decode_welcome(frame.payload).player_id;
  ...
  case proto::MessageType::movement_state:
    return proto::decode_movement_state(frame.payload).player_id;
  case proto::MessageType::jump_event:
    return proto::decode_jump_event(frame.payload).player_id;
  case proto::MessageType::player_joined:
    return proto::decode_player_joined(frame.payload).player_id;
  case proto::MessageType::player_left:
    return proto::decode_player_left(frame.payload).player_id;
  default:
    return std::nullopt;
  }
}
...
public:
  ...
  auto connect_and_hello() -> void {
    ...
    if (socket_ == eo::net::kInvalidSocket)
      throw std::runtime_error(name_ + " connect failed: " + error);
    eo::net::set_no_delay(socket_, true, error);
    send(proto::make_frame(
        proto::MessageType::hello, sequence_++,
        proto::Hello{name_, eo::build_info::identity(
                                "SelfTest", proto::kProtocolVersion)}));
    const auto welcome = wait_for(proto::MessageType::welcome, std::nullopt,
  ...
  auto send(const proto::Frame &frame) -> void {
    ...
    std::string error;
    if (!eo::net::send_all(socket_, bytes, error))
      throw std::runtime_error(name_ + " send failed: " + error);
  }
  ...
  auto wait_for(proto::MessageType type, std::optional<std::uint64_t> player_id,
    ...
    while (std::chrono::steady_clock::now() < deadline) {
      const auto found =
          std::find_if(pending_.begin(), pending_.end(), matches);
    ...
    throw std::runtime_error(name_ + " timed out waiting for " +
                             proto::message_type_name(type));
  }
  ...
  auto close_reset() -> void {
    ...
    setsockopt(socket_, SOL_SOCKET, SO_LINGER,
               reinterpret_cast<const char *>(&reset_linger),
               sizeof(reset_linger));
    eo::net::close_socket(socket_);
  }

  auto expect_absent(proto::MessageType type,
                     std::optional<std::uint64_t> player_id,
                     std::chrono::milliseconds duration) -> void {
    const auto matches = [&](const proto::Frame &frame) {
      return frame.type == type &&
             (!player_id || player_id_of(frame) == player_id);
    };
    const auto deadline = std::chrono::steady_clock::now() + duration;
    while (std::chrono::steady_clock::now() < deadline) {
      if (std::any_of(pending_.begin(), pending_.end(), matches))
        throw std::runtime_error(name_ + " received duplicate " +
                                 proto::message_type_name(type));
      receive_once(25);
    }
    if (std::any_of(pending_.begin(), pending_.end(), matches))
...
private:
  auto receive_once(int timeout_ms) -> void {
    ...
    if (!eo::net::wait_readable(socket_, timeout_ms, error)) {
      if (!error.empty())
        throw std::runtime_error(name_ + " wait failed: " + error);
      return;
    }
    std::array<std::uint8_t, 64U * 1024U> buffer{};
    const auto received = eo::net::receive_some(socket_, buffer, error);
    if (received <= 0)
      throw std::runtime_error(name_ + " connection closed: " + error);
    decoder_.push(std::span<const std::uint8_t>(
        buffer.data(), static_cast<std::size_t>(received)));
    for (auto &frame : decoder_.take_frames()) {
      if (frame.version != proto::kProtocolVersion)
        throw std::runtime_error("protocol mismatch during self-test");
      if (frame.type == proto::MessageType::ping) {
        send(proto::Frame{proto::kProtocolVersion,
...
auto verify_malformed_isolation(const Options &options, TestClient &observer)
    -> void {
  std::string error;
  auto socket = eo::net::connect_tcp(options.host, options.port, error);
  if (socket == eo::net::kInvalidSocket)
    throw std::runtime_error("malformed client could not connect: " + error);
  const auto illegal = proto::make_frame(proto::MessageType::zone_state, 1,
                                         proto::ZoneState{999999, "ZoneA"});
  const auto bytes = proto::encode_frame(illegal);
  if (!eo::net::send_all(socket, bytes, error))
    throw std::runtime_error("malformed packet send failed");
  if (!eo::net::wait_readable(socket, 2000, error))
    throw std::runtime_error("malformed client was not rejected");
  std::array<std::uint8_t, 4096> buffer{};
  const auto received = eo::net::receive_some(socket, buffer, error);
  eo::net::shutdown_socket(socket);
  eo::net::close_socket(socket);
  if (received <= 0)
    throw std::runtime_error("malformed rejection had no diagnostic frame");
  ...
  unknown_type.send(proto::Frame{proto::kProtocolVersion,
                                 ...
                                 500,
                                 {}});
  (void)unknown_type.wait_for(proto::MessageType::error, std::nullopt,
                              std::chrono::seconds(2));

  TestClient wrong_protocol(options.host, options.port,
                            "SelfTest-WrongProtocol");
  wrong_protocol.connect_and_hello();
  wrong_protocol.send(
      proto::Frame{static_cast<std::uint16_t>(proto::kProtocolVersion - 1),
                   proto::MessageType::ping,
                   600,
                   {}});
  (void)wrong_protocol.wait_for(proto::MessageType::error, std::nullopt,
                                std::chrono::seconds(2));

  TestClient invalid_movement(options.host, options.port,
                              "SelfTest-InvalidMovement");
...
auto run_self_test(const Options &options) -> void {
  ...
  alice.send_message(proto::MessageType::zone_state,
  ...
  bob.send_message(proto::MessageType::transform_snapshot,
                   proto::TransformSnapshot{888, 1100, 400.0F, 500.0F, 600.0F,
                                            0.0F, 90.0F, 0.0F});
  ...
  (void)bob.wait_for(proto::MessageType::player_joined, alice.id(),
                     std::chrono::seconds(3));
  (void)bob.wait_for(proto::MessageType::zone_state, alice.id(),
                     std::chrono::seconds(3));
  ...
  (void)alice.wait_for(proto::MessageType::player_joined, bob.id(),
                       std::chrono::seconds(3));
  ...
  (void)alice.wait_for(proto::MessageType::transform_snapshot, bob.id(),
                       std::chrono::seconds(3));
  (void)alice.wait_for(proto::MessageType::movement_state, bob.id(),
                       std::chrono::seconds(3));

  alice.send_message(proto::MessageType::jump_event, proto::JumpEvent{999, 1});
  const auto first_jump = bob.wait_for(proto::MessageType::jump_event,
                                       alice.id(), std::chrono::seconds(3));
  const auto decoded_first_jump = proto::decode_jump_event(first_jump.payload);
  if (decoded_first_jump.player_id != alice.id() ||
      decoded_first_jump.sequence != 1)
    throw std::runtime_error("JumpEvent was not server-authoritative");
  alice.send_message(proto::MessageType::jump_event,
                     proto::JumpEvent{123456, 1});
  bob.expect_absent(proto::MessageType::jump_event, alice.id(),
                    std::chrono::milliseconds(300));
  alice.send_message(proto::MessageType::jump_event,
                     proto::JumpEvent{123456, 2});
  const auto second_jump = bob.wait_for(proto::MessageType::jump_event,
                                        alice.id(), std::chrono::seconds(3));
  ...
  (void)alice.wait_for(proto::MessageType::player_left, bob.id(),
                       std::chrono::seconds(3));
  bob.send_message(proto::MessageType::zone_state,
                   proto::ZoneState{bob.id(), "ZoneA"});
  (void)alice.wait_for(proto::MessageType::player_joined, bob.id(),
                       std::chrono::seconds(3));
  (void)bob.wait_for(proto::MessageType::player_joined, alice.id(),
                     std::chrono::seconds(3));
  ...
  alice.close_reset();
  (void)bob.wait_for(proto::MessageType::player_left, old_alice_id,
                     std::chrono::seconds(4));

  TestClient reconnected(options.host, options.port, "SelfTest-A-Reconnected");
  reconnected.connect_and_hello();
  if (reconnected.id() == old_alice_id)
    throw std::runtime_error("reconnect reused a stale session id");
  ...
  reconnected.send_message(proto::MessageType::transform_snapshot,
                           proto::TransformSnapshot{old_alice_id, 2000, 700.0F,
                                                    800.0F, 900.0F, 0.0F, 0.0F,
                                                    0.0F});
  ...
  silent.send_message(proto::MessageType::zone_state,
                      proto::ZoneState{0, "ZoneA"});
  (void)bob.wait_for(proto::MessageType::player_joined, silent.id(),
                     std::chrono::seconds(3));
  (void)bob.wait_for(proto::MessageType::player_left, silent.id(),
