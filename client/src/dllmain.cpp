...
auto mod_root() -> std::filesystem::path
{
    ...
    const auto size = GetModuleFileNameW(reinterpret_cast<HMODULE>(&__ImageBase), path.data(), static_cast<DWORD>(path.size()));
    if (size == 0 || size == path.size()) throw std::runtime_error("GetModuleFileNameW failed");
    path.resize(size);
...
class ExpeditionOnlineMod final : public RC::CppUserModBase
{
  public:
    ...
    ~ExpeditionOnlineMod() override
    {
        ...
        network_.reset();
        logger_.reset();
        RC::Output::send<RC::LogLevel::Verbose>(STR("[ExpeditionOnline] native mod unloaded\n"));
    }
    ...
    auto on_unreal_init() -> void override
    {
        try
        {
            ...
            if (hook_id_ == RC::Unreal::Hook::ERROR_ID)
            {
                throw std::runtime_error("could not register ProcessEvent game-thread callback");
            }
            RC::Output::send<RC::LogLevel::Verbose>(STR("[ExpeditionOnline] initialized; see ExpeditionOnline.log\n"));
        }
        catch (const std::exception& exception)
        {
            if (logger_) logger_->error(std::string("INIT_FAILED ") + exception.what());
            RC::Output::send<RC::LogLevel::Error>(STR("[ExpeditionOnline] initialization failed\n"));
        }
    }
