...
class Writer
{
  public:
    ...
    {
        if (value.size() > kMaxStringSize || value.size() > std::numeric_limits<std::uint16_t>::max())
        {
    ...
    auto take() && -> std::vector<std::uint8_t>
    {
...
auto read_u16_at(std::span<const std::uint8_t> bytes, std::size_t offset) -> std::uint16_t
{
    return static_cast<std::uint16_t>((static_cast<std::uint16_t>(bytes[offset]) << 8U) | bytes[offset + 1]);
}

auto read_u32_at(std::span<const std::uint8_t> bytes, std::size_t offset) -> std::uint32_t
{
    std::uint32_t value{};
...
auto read_u64_at(std::span<const std::uint8_t> bytes, std::size_t offset) -> std::uint64_t
{
    std::uint64_t value{};
...
{
    Reader reader(bytes);
    AppearanceState value{reader.u64(), reader.string(), reader.string(), reader.string()};
    reader.finish();
    return value;
}
...
auto FrameDecoder::push(std::span<const std::uint8_t> bytes) -> void
{
    ...
    {
        const std::span<const std::uint8_t> remaining(buffer_.data() + consumed, buffer_.size() - consumed);
        ...
        frame.payload.assign(remaining.begin() + static_cast<std::ptrdiff_t>(kHeaderSize),
...
auto message_type_name(MessageType type) -> const char*
{
    ...
    case MessageType::zone_state: return "ZoneState";
    case MessageType::appearance_state: return "AppearanceState";
    case MessageType::transform_snapshot: return "TransformSnapshot";
